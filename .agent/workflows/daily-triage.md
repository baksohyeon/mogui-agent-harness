...
**Review-evidence signal (per open PR, read-only).** For each open PR branch, check whether a local review log exists: `~/.gstack/projects/<slug>/<branch>-reviews.jsonl` (branch name normalized by stripping `/`, e.g. `chore/x` → `chorex`). Present → `/review`/`/review-pr` ran on this machine; absent → **no local review evidence**. This is a **local, per-machine** signal: a PR reviewed on another machine won't show here — report "no local review evidence", never assert "not reviewed".

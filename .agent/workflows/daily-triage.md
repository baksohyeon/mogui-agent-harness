...
**Review-evidence signal (per open PR, read-only).** For each open PR branch, check whether a local review log exists: `~/.gstack/projects/<slug>/<branch>-reviews.jsonl`. The branch key uses the gstack-slug encoding — every character outside `[a-zA-Z0-9._-]` is removed (e.g. `chore/x` → `chorex`). This encoding is lossy and can collide (`feature/x` and `featurex` share a key), so treat file presence as a heuristic; when in doubt, cross-check the entries' `commit` field against the PR head SHA before attributing evidence. Present → `/review`/`/review-pr` ran on this machine; absent → **no local review evidence**. This is a **local, per-machine** signal: a PR reviewed on another machine won't show here — report "no local review evidence", never assert "not reviewed".

**Harness-file-change signal (per open PR, read-only).** For each open PR/MR, fetch its touched file list via the repo's forge CLI — GitHub: `gh pr view <N> --json files`; GitLab: `glab mr diff <N>` (read-only; take the touched paths from the diff headers). Apply the same path globs to either result: paths under `.gitignore`·`.agent/**`·`.claude/**`·`.codex/**`·`scripts/**`·`templates/**` → record `- [pr] #N touches harness files → recommend human review`. This gathers samples of personal/local state leaking into the shared harness.

...
| Host hooks (Claude/Codex) | Health / bootstrap / style — **warnings to the session**, not durable records | **Wired** in `.claude/settings.json` / `.codex/hooks.example.json` | SessionStart: code-review-graph status, setup-check, memory-health, load-context, wiki-health; UserPromptSubmit: memory-selfcheck, remind-korean-style |

# agentic-starter: 복사해서 바로 쓰는 에이전트 운영 starter

> 한국어 문서입니다. English: [README.md](./README.md)
>
> **원본 프로젝트:** 이 starter는 **agent-harness**(`dorito-agent-harness`)에서 제품 결정·회고를 전부 걷어내고 뽑아낸 **빈 템플릿**입니다. agent-harness가 이 운영 레이어를 실제로 굴리는 정본(설명 + 결정 이력 포함)이고, 여기선 골격만 받습니다.
...
starter 원형을 커밋으로 남기고 싶다면 1단계 직후 바로 커밋합니다.

```bash
git add .
git commit -m "chore: initialize agentic starter"
```

그다음 2단계로 빈칸을 채웁니다. 이렇게 하면 "starter를 처음 깐 커밋"과 "우리 프로젝트 값으로 채운 커밋"이 나뉩니다. 나중에 원형과 프로젝트별 수정분을 비교하기 쉽습니다.
...
- [Onboarding](./docs/wiki/guides/01-onboarding.md): 사람·새 AI 세션의 시작점
- [Agentic Architecture](./docs/wiki/guides/02-agentic-architecture.md): `.agent` / `.planning` / `docs/wiki` 분리, 상세
- [Host Runtime and Hooks](./docs/wiki/guides/03-host-runtime-and-hooks.md): Claude Code·Codex·Cursor가 컨텍스트를 읽고 hook을 도는 법
- [GSD + LLM Wiki Agent Flow](./docs/wiki/guides/04-gsd-llm-wiki-agent-flow.md): 요청이 진행 상태와 장기 지식 사이를 라우팅되는 법
- [Decisions and Postmortems](./docs/wiki/guides/05-decisions-and-postmortems.md): ADR(결정 기록)과 postmortem 쓰는 법
- [Maintenance](./docs/wiki/guides/maintenance.md): 운영 문서를 오래 유지하는 법
...
| `scripts/` | setup, git hook install, 골격 wiring verify |
| `PROMPTS.md` | 첫 실행과 운영 점검 프롬프트 |
| `docs/bootstrap-walkthrough.md` | 빈 repo에서 첫 결정까지 한 세션을 따라간 worked example |
...
| `setup-check.sh` | 유지 | `.setup-done`이 없으면 `[AGENT-ASK]`로 setup 실행 여부를 묻게 합니다. |
